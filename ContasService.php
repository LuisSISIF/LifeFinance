    ...
    public static function getDadosPaginaContas(PDO $pdo, int $userId): array {
        $dados = [
            ...
            'painelLateral' => [
                'contaPrincipal' => null,
                'resumoContas' => [],
                'metaSaldo' => ['valor_meta' => 15000, 'nome' => 'Objetivo mensal', 'pct' => 0, 'falta' => 15000],
            ],
        ...
        $stmt = $pdo->prepare('SELECT SUM(saldo) as saldo_total, COUNT(id) as qtd_ativas, MAX(ultima_sincronizacao_em) as ultima_sync FROM contas WHERE id_usuario = ? AND ativa = 1');
        $stmt->execute([$userId]);
        if ($resumo = $stmt->fetch()) {
            ...
            if (!empty($resumo['ultima_sync'])) {
                // Formata se foi hoje, ontem ou data
                $dataSync = new DateTime($resumo['ultima_sync']);
                $hoje = new DateTime('today');
                $diff = $hoje->diff(new DateTime($dataSync->format('Y-m-d')))->days;
                if ($diff === 0) $dados['ultimaSincronizacao'] = 'Hoje';
                elseif ($diff === 1) $dados['ultimaSincronizacao'] = 'Ontem';
                else $dados['ultimaSincronizacao'] = $dataSync->format('d/m/Y');
            }
        }

        // 2. Cartões Vinculados
        $stmt = $pdo->prepare('SELECT COUNT(c.id) as qtd_cartoes FROM cartoes c JOIN contas a ON c.id_conta = a.id WHERE a.id_usuario = ?');
        $stmt->execute([$userId]);
        ...
        // 4. Painel Lateral (Conta principal e top 2 outras contas)
        $painelContas = [];
        foreach ($dados['listaContas'] as $conta) {
            $pct = $dados['saldoConsolidado'] > 0 ? round(($conta['saldo'] / $dados['saldoConsolidado']) * 100) : 0;
            $contaData = [
                'nome' => $conta['nome'],
                'saldo' => $conta['saldo'],
                'pct' => $pct,
        ...
        // Ordenar as restantes por saldo desc e pegar as top 2
        usort($painelContas, function($a, $b) { return $b['saldo'] <=> $a['saldo']; });
        $dados['painelLateral']['resumoContas'] = array_slice($painelContas, 0, 2);

        // 5. Meta de Saldo
        $stmt = $pdo->prepare('SELECT nome, valor_meta FROM metas_financeiras WHERE id_usuario = ? AND ativa = 1 ORDER BY reserva_emergencia DESC, valor_meta DESC LIMIT 1');
        $stmt->execute([$userId]);
        if ($meta = $stmt->fetch()) {
        ...
        // 6. Limite de Crédito
        $stmt = $pdo->prepare('SELECT SUM(limite) as limite_total FROM cartoes c JOIN contas a ON c.id_conta = a.id WHERE a.id_usuario = ?');
        $stmt->execute([$userId]);
        $limiteTotal = (float)$stmt->fetchColumn();
        
        // Buscar gastos de cartões no mês atual
        $mesAtual = date('Y-m');
        $stmt = $pdo->prepare("
        ...
        // 7. Saldo Projetado
        // saldo atual + (contas receber mês) - (contas pagar mês)
        $stmtReceber = $pdo->prepare("SELECT SUM(valor_total) FROM contas_receber WHERE id_usuario = ? AND status IN ('PENDENTE', 'VENCIDO') AND DATE_FORMAT(vence_em, '%Y-%m') = ?");
        $stmtReceber->execute([$userId, $mesAtual]);
        $aReceber = (float)$stmtReceber->fetchColumn();
        
        $stmtPagar = $pdo->prepare("SELECT SUM(valor_total) FROM contas_pagar WHERE id_usuario = ? AND status IN ('PENDENTE', 'VENCIDO') AND DATE_FORMAT(vence_em, '%Y-%m') = ?");
        $stmtPagar->execute([$userId, $mesAtual]);
        $aPagar = (float)$stmtPagar->fetchColumn();

        $projetado = $dados['saldoConsolidado'] + $aReceber - $aPagar;
        if ($projetado > 0) $dados['saldoProjetadoStatus'] = 'Positivo';
        elseif ($projetado < 0) $dados['saldoProjetadoStatus'] = 'Negativo';
        else $dados['saldoProjetadoStatus'] = 'Zerado';
